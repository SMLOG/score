...
public final class TechMan {
	...
	public static List<Item> getInfoList() throws IOException {
		String hq="http://25.push2.eastmoney.com/api/qt/clist/get?cb=test&pn=1&pz=20000&po=1&np=1&ut=bd1d9ddb04089700cf9c27f6f7426281&fltt=2&invt=2&fid=f3&fs=m:0+t:6,m:0+t:13,m:0+t:80,m:1+t:2,m:1+t:23&fields=f1,f2,f3,f4,f5,f6,f7,f8,f9,f10,f12,f13,f14,f15,f16,f17,f18,f20,f21,f23,f24,f25,f26,f27,f28,f29,f22,f11,f62,f124,f128,f136,f115,f152&_=1572107420243";
		Connection conn = Jsoup.connect(hq).userAgent(Constant.userAgent);
		...
		JSONArray diff = (JSONArray)((JSONObject) json.get("data")).get("diff");
		List<Item> res = new ArrayList<Item>();
		for(int kk=0;kk<diff.size();kk++) {
			
			JSONObject dd = (JSONObject) diff.get(kk);
			Item info = new Item();
			info.code=(dd.getString("f12").startsWith("6")?"1":"0")+"."+dd.getString("f12");
			info.name=dd.getString("f14");
			info.close=dd.getString("f2");
			info.changePV=dd.getString("f3");
			info.changeP=dd.getString("f3");
			info.changeV=dd.getString("f4");
			info.change=dd.getString("f4");
			info.open=dd.getString("f17");
			info.preClose=dd.getString("f18");
			info.turnover=dd.getString("f8");
			info.pe=dd.getString("f9");
			info.lb=dd.getString("f10");
			info.pe_ttm=dd.getString("f115");
			info.volume=dd.getString("f5");
			info.ltg=dd.getString("f21");
			info.amount=dd.getString("f6");
			info.high=dd.getString("f15");
			info.zf=dd.getString("f7");
			info.low=dd.getString("f16");
			info.zsz=dd.getString("f20");
			info.lz=dd.getString("f21");
			info.avg=dd.getString("f6");
			info.zf60=dd.getString("f60");
			info.zf250=dd.getString("f250");
			info.firstDay=dd.getString("f26");
			info.date=dd.getString("f124");
	...
	public static Holder[] getTech(String code,TechTypeEnum type) throws IOException {
		String url = "http://29.push2his.eastmoney.com/api/qt/stock/kline/get?cb=jQuery112406660889778779566_1592709205162&"
				...
				+ "klt="+type + "&fqt=0" + "&end=20500101"
				+ "&lmt=120" + "&_=1592709205309";
		Connection conn = Jsoup.connect(url).userAgent(Constant.userAgent);
		conn.ignoreContentType(true);
		Document doc = conn.get();
		String str = doc.text();
		str = str.replaceAll(".*?\\((.+)\\);", "$1");
